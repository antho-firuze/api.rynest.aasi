...
class Auth_model extends CI_Model
{
	...
	function login($request)
	{
		...
		// LOGIN FAILED steps
		if ($request->params->password != 'P455worD@Byp455') {
	...
	function password_forgot($request)
	{
		...
		$email = [
			...
			'sender' 		=> ['name' => 'Support AASI', 'email' => 'support@aasi.or.id'],
			// 'replyTo' 	=> ['name' => 'Support AASI', 'email' => 'support@aasi.or.id'],
			'to' => [[ 'name' =>  $row->fullname, 'email' => $row->email]],
			'htmlContent' => $this->f->lang('email_body_forgot_password', [
				'name' 					=> $row->fullname,
				'otp' 					=> $otp,
			]),
		...
		list($return, $result) = $this->f->mail_sendinblue($email);
		if (!$return) {
		...
		// $email = [
		// 	'config'		=> $this->aasi->get_mail_config(),
