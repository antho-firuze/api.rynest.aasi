...
define('CDN_PHOTO_URL', "https://cdn.lsp-ps.id/assets/img/relax/");
...
class Lsp
{
	...
	function get_company($company_id) // repaired
	{
		...
		if (FALSE === $row = $ci->f->get_db_row('companies'))
			return [FALSE, $ci->f->error()];
	...
	function get_member_schedule($member_id, $company_id)	// repaired
	{
		$ci = &get_instance();
		$ci->load->library('f');

		$str = "(
			select t1.schedule_request_id, t2.schedule_id, t2.location_id, 
			t3.category_id, t3.name, t3.date, t3.pre, t3.begin, t3.duration, t3.notes, 
			...
			where t1.member_id = ? and t2.company_id = ? 
			order by date desc 
			limit 1
		) g0";
		$table = $ci->f->compile_qry($str, [$member_id, $company_id]);
		if (FALSE === $row = $ci->f->get_db_row($table))
			return [FALSE, $ci->f->error()];
	...
	function get_member_photo($member_id, $schedule_request_id)	// repaired
	{
		$ci = &get_instance();
		$ci->load->library('f');
		...
		$str = "(
			...
			where member_id = ? and schedule_request_id = ? 
		) g0";
		$table = $ci->f->compile_qry($str, [$member_id, $schedule_request_id]);
		...
		foreach ($rows as $key => $val) {
			$sub_folder = $val->schedule_request_id;
			$filename 	= $val->filename;
			$type				= $val->type;
			$link[$type] = $base_path . 'schedule_request_id/' . $sub_folder . "/$filename";
		}

		return [TRUE, count($link) < 1 ? null : $link];
	}

	function get_exam_results($schedule_request_id, $member_id)	// repaired
	{
		$ci = &get_instance();
		$ci->load->library('f');
		...
		$table = $ci->f->compile_qry($str, [$schedule_request_id, $member_id]);
	...
	function get_exam_start_logs($schedule_request_id, $member_id)	// repaired
	{
		$ci = &get_instance();
		$ci->load->library('f');
	...
	function get_exam_finish_logs($schedule_request_id, $member_id)	// repaired
	{
		$ci = &get_instance();
		$ci->load->library('f');
	...
	function is_exam_still_running($request, $schedule_request_id, $member_id)
	{
		$ci = &get_instance();
		$ci->load->library('f');
		...
		// GET exam start logs
		list($return, $result) = $this->get_exam_start_logs($schedule_request_id, $member_id);
		if (!$return) return [FALSE, $result];
		...
		if ($dt_client >= date_create($state->exam_end)) {
			...
			list($return, $result) = $this->set_exam_finish($request, $schedule_request_id, $member_id);
			if (!$return) return [FALSE, $result];
	...
	function set_exam_finish($request, $schedule_request_id, $member_id)
	{
		$ci = &get_instance();
		$ci->load->library('f');
		...
		list($return, $result) = $this->get_exam_finish_logs($schedule_request_id, $member_id);
		if (!$return) return [FALSE, $result];
	...
	function get_mail_config()
	{
		return [
			...
			'smtp_timeout' => 7,
			'from'				=> 'support@lsp-ps.id',		// email sender
			'from_name'		=> 'LSP Perasuransian Syariah',
		];
	}
